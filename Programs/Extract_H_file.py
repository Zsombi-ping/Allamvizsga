# Extracting Handcrafted features from each file
...
words = []
pathDir = folderpath # don't forget to point this to your data dir!
DIRSEPARATOR = '\\'
...
def struct_to_file(words):
    with open(typetxt,'a') as words_file:
        for i in words:
            with open(typetxt,'r') as words_in_file:
...
def define_to_file(words):
    with open(definetxt,'a') as words_file:
        for i in words:
            with open(definetxt,'r') as words_in_file:
                if not (i in words_in_file.read()):
                    words_file.write('%s\n'%i)


def function_to_file(words):
    with open(functiontxt,'a') as words_file:
        for i in words:
            with open(functiontxt,'r') as words_in_file:
                if not (i in words_in_file.read()):
                    words_file.write('%s\n'%i)


def variable_to_file(words):
    with open(variabletxt,'a') as words_file:
        for i in words:
            with open(variabletxt,'r') as words_in_file:
                if not (i in words_in_file.read()):
                    words_file.write('%s\n'%i)
...
def dump(data):
    ...
    index_counter = 1
    global print_header
    csv_file = GCJ_100_handcrafted_file
    ...
    if print_header:
        ...
        df.to_csv(csv_file, mode = 'w',float_format='%.2f' ,header=csv_columns, index = False) #with headers
        print_header = False
    else:
        df.to_csv(csv_file, mode = 'a',float_format='%.2f', header = False, index = False) #append mode without headers
...
def extract_features(filepath, author, creation_order, csv_flag):
    ...
    with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
        ...
        feature_map['Custom Type Name Readability'] = count_readability(file_to_list(typetxt))
        ...
        feature_map['Variable Name Readability'] = count_readability(file_to_list(variabletxt))
        feature_map['Define Name Readability'] = count_readability(file_to_list(definetxt))
...
main(folderpath, 'WINDOWS', 'YES_CSV')
